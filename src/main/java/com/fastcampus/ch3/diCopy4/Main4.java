...
class AppContext{
        ...
        try {
            for(Object bean : map.values()){
                for(Field fid : bean.getClass().getDeclaredFields()){
                    if(fid.getAnnotation(Resource.class)!=null) fid.set(bean,getBean(fid.getName()));
                }
            }
    ...
    private void doAutowired() {
        ...
        try {
            for(Object bean : map.values()){
                for(Field fid : bean.getClass().getDeclaredFields()){
                    if(fid.getAnnotation(Autowired.class)!=null) fid.set(bean,getBean(fid.getType()));
                }
            }
        } catch (IllegalAccessException e) {
            throw new RuntimeException(e);
...
public class Main4 {
    public static void main(String[] args) throws Exception{
        ...
        Engine engine = (Engine) ac.getBean("enigne");
